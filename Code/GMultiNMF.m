function [t,U,V,centroidV,wt,alphas,acc,nmi,Pi,Ri,Fi,ARi,l] = GMultiNMF(X, K, W, gnd,options)
%	Notation:
...
while jjj < Rounds  % ================ outer loop ===============
    ...
    centroidV = centroidV / sum(alphas.^p);
    % ================ update wt ========================
    wt_new=cell(1,viewNum);
    for i=1:viewNum
        Y{i}=X{i}-U{i}*V{i}'; 
    end
    
    for j=1:viewNum
        A_temp=options.beta/2*except_j(j,nSmp,X{i},V{i},wt{j});
        wt_new{j}=diag(ones(nSmp,1));
        for k=1:nSmp
            result=wtn(k,Y);
            Ysum=sum(Y{j}(:,k).^2);
            wt_new{j}(k,k)=((1+A_temp/2/result)/2/result-A_temp)/2/Ysum;
        end
    end
    wt=wt_new;
    % =================== Update A ==================
