...
%
%   GLYPHPLOT(X, ..., 'ObsLabels',OBSLABS) labels each glyph with the text
%   in the string array, character array or cell array of strings OBSLABS.
%   By default, the glyphs are labelled 1:N.  Pass in '' for no labels.
%   Labels are not plotted when the 'Centers' parameter is given, but
%   appear only in data cursors.
%
%   GLYPHPLOT(X, ..., 'VarLabels',VARLABS) uses the text in the string
%   array, character array or cell array of strings VARLABS as variable
%   names when displaying data cursors.
%
...
if nargout > 0
    ...
    if showIndividualVar
        ...
        dataCursorText{offset+1} = [varlabs{clickVarNum} ': ' convertToString(X(clickObsNum,clickVarNum))];
    ...
    if dfltObslabs
        obsText = getString(message('stats:glyphplot:Observation', convertToString(clickObsNum)));
    else
    ...
    for j = 1:p
        dataCursorText{offset+j} = ...
            [varlabs{j} ': ' convertToString(X(clickObsNum,j))];
    end
    %offset = offset+p;
