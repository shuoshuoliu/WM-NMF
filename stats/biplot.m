...
if in3D
    ...
    varHndl = [line(cax,arx(1:2,:),ary(1:2,:), 'Color','b', 'LineStyle','-', plotArgs{:}, 'Marker','none'); ...
               line(cax,arx(2:3,:),ary(2:3,:), 'Color','b', 'Marker','.', plotArgs{:}, 'LineStyle','none')];
end
...
if ~isempty(varlabs)
    ...
    if in3D
        varTxtHndl = text(cax,coefs(:,1)+delx,coefs(:,2)+dely,coefs(:,3)+delz,varlabs,'Interpreter','none');
    else
        varTxtHndl = text(cax,coefs(:,1)+delx,coefs(:,2)+dely,varlabs,'Interpreter','none');
    end
