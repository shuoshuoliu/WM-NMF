...
%                  is 1 if variable i is nested in variable j. Nesting is
%                  not allowed in a continuous variable.
...
if ~isempty(vnested)
   ...
   % Must not have nesting in a continuous variable
   if any(continuous(nester))
       error('stats:anovan:NestedInContinuous','Variables cannot be nested in a continuous variable.')
   end
end
end
...
for j=1:ng
   ...
   elseif isempty(vnested) || ~any(vnested(j,:))
      ...
      idum = idummy(allgrps(:,j), 3);
      if ~warned && cattype(j) && any(all(idum==0,1))
          % If a categorical predictor has a column of all zeros,
          % this means one of its categories is missing
          warning(message('stats:anovan:DropLevels',j))
          warned = true;
      end
      vdum{j} = idum;
