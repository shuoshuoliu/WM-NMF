...
%                           NLMEFITSA calls all output functions after each
%                           iteration. See NLMEFITOUTPUTFCN (default) for
...
if isempty(v)
    v = zeros(NGroups,0);
elseif ~ismatrix(v) || size(v,1) ~= max(Id) || ...
        ~((isnumeric(v) && isreal(v)) || iscell(v))
    error(message('stats:nlmefitsa:BadV'));
elseif size(v,1)>NGroups
