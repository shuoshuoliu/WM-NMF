...
elseif iscell(group)
    ...
    % If inputs through cells are tall arrays, pass to tall/dummyvar. If
    % one input is tall, all others must also be tall and created from the
    % same datastore
    inpTall = cellfun(@(x) istall(x),group);
    if all(inpTall)
        dCell = cell(1,n);
        for idx = 1:n
            dCell{idx} = dummyvar(group{idx},true);            
        end
        D = horzcat(dCell{:});
        return;
    elseif any(inpTall) && ~all(inpTall) 
        error(message('stats:dummyvar:NotAllTall'));
    end
