...
newfigure = (nargin<6 || isempty(infigh));
if newfigure
...
height = resizetable(infigh);

% Preserve default position of the top of the figure, but adjust height
newpos = infigh.Position;
height = min(height,newpos(4));
newpos(2) = newpos(2) + newpos(4)-height;
newpos(4) = height;
infigh.Position = newpos;
...
%-----------------------
function height = resizetable(fig)
...
% Compute height of figure contents
height = 1.1*(capheight+headheight+u.Extent(4));
