...
allax = gobjects(1,nplots);
if dodisp && isempty(parent)
    clf;
end
if nargout>=2
    plotdata = struct('pts',num2cell(1:nplots)); % initialize to proper size
end
...
if dodisp
    ...
    try
        ...
        if ~strcmp(me.identifier,'MATLAB:UndefinedFunction') && ...
           ~strcmp(me.identifier,'MATLAB:ui:uifigure:UnsupportedAppDesignerFunctionality')
            % If ax is a uiaxes handle, we expect this error
            rethrow(me)
        end
    end
end
...
% --------------------------------------
function txt = makedcursor(~,event)
...
if isempty(nestedstr.label) % use label if provided
    ...
    nestedlbl = [nestedstr.label{nestedj},newline];
end
...
if ~isempty(limits)
    lcl = repmat(limits(1),size(pts));
    cl  = repmat(limits(2),size(pts));
    ucl = repmat(limits(3),size(pts));
    se = 0;
end
...
hold(ax,'on');
h(2) = plot(ax,ind(1),pts(1),'ro','hittest','off','PickableParts','none','tag','ooc');
set(h(2),'xdata',ind(ooc),'ydata',pts(ooc));

% Now plot center line and control limits
h(3) = plotline(ind,cl,'-','color',[0 .75 0],'parent',ax,'hittest','off','PickableParts','none','tag','cl');
h(4) = plotline(ind,[lcl ucl],'r-','parent',ax,'hittest','off','PickableParts','none','tag','lclucl');
...
if ~isempty(specs) && ~ismember(cctype,{'r' 's' 'mr'})
    h(5) = plotline(ind,specs(:)','r:','parent',ax,'hittest','off','PickableParts','none','tag','specs');
