...
%   TABLE = TABULATE(X) takes a numeric vector X and returns a numeric
%   matrix, TABLE. The first column of TABLE contains the unique values of
%   X.  The second is the number of instances of each value.  The last
%   column contains the percentage of each value.  If the elements of X are
%   positive integers, then the output includes 0 counts for any integers
%   that are between 1 and max(X) but do not appear in X.
%
%   TABLE = TABULATE(X), where X is a logical vector, categorical variable,
%   character array, string array, or a cell array of strings, returns
%   TABLE as a cell array.  The first column contains the unique string
%   values in X, and the other two columns are as above.
%
