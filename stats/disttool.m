...
%   distributions in the Statistics and Machine Learning Toolbox.
%
...
if strcmp(action,'start'),
   ...
   axisp   = [.23 .35 .75 .55];
   pos = cell(7,3);
   pos{6,1} = [.10 .15 .12 .05];   % upper bound label
   pos{5,1} = [.14 .075 .08 .05];   % text
   pos{7,1} = [.10 .01 .12 .05];   % lower bound label
   ...
   pos{6,2} = [.41 .15 .12 .05];   % upper bound label
   pos{5,2} = [.445 .075 .08 .05];   % text
   pos{7,2} = [.41 .01 .12 .05];   % lower bound label
   ...
   pos{6,3} = [.71 .15 .12 .05];   % upper bound label
   pos{5,3} = [.75 .075 .08 .05];   % text
   pos{7,3} = [.71 .01 .12 .05];   % lower bound label
...
switch ud.dists(popupvalue).rvname
    ...
    case 'hnrv', % Half Normal
       ...
       xrange  = [plo(1) hixvalue];       
       % a fine grid to make sure we get close to the mode at mu for any mu
       xvalues = linspace(plo(1),hixvalue,5001);  
