...
p=schema.prop(c, 'name', 'ustring');                %#ok<*NASGU> % fit name
p=schema.prop(c, 'dataset', 'ustring');             % dataset name
...
if (s.stats.DistributionFitter.LegacyDistributionFitter.ActiveValue == false)
    p=schema.prop(c, 'fitframe', 'MATLAB array'); % dfittool gui new/edit FitEditor.m class
else
