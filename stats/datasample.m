...
if ~defaultStream
    ...
    % helpful check for RNG output being passed in
    if isscalar(x) && isstruct(x)
        error(message('stats:datasample:InvalidStream'));
    end
end
...
if nargs > 2 && ~(ischar(varargin{1}) || isstring(varargin{1}))
    dim = varargin{1};
    varargin(1) = [];
    nargs = nargs - 1;
else
...
validateattributes(dim,{'numeric'},{'scalar'},'','dim'); % size() checks for other errors
n = size(x,dim);
...
if replace
    ...
    elseif isempty(wgts) % unweighted sample
        ...
        if defaultStream
            i = internal.stats.wswor(wgts,k);
        else
            i = internal.stats.wswor(s,wgts,k);
        end
    end
end
