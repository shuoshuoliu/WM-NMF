...
%   X = GEOINV(Y,P) returns the inverse cdf of the geometric distribution
...
if all(k(:))
    ...
    numer = nan(size(y),'like',internal.stats.dominantType(y,p)); % single if y or p is
    numer(k) = log(1-y(k));
    numer(y == 0) = 0;
    numer(y == 1) = -Inf; % avoid log(0) warnings
end

peps = eps(ones('like',p));
k = (peps < p & p < 1);
if all(k(:))
    denom = log(1-p);
else
    % Return NaN for out of range parameters: p<=0 | 1<p | isnan(p).
    denom = NaN(size(p),'like',p);
