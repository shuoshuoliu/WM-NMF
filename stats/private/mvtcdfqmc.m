...
outclass = internal.stats.dominantType(a,b,Rho,nu);
...
if ~all(a < b)
    ...
    elseif any(isnan(a) | isnan(b))
        p = NaN('like',outclass);
        err = NaN('like',outclass);
    else % any(a == b),this includes +/- Inf
        p = zeros('like',outclass);
        err = zeros('like',outclass);
    end
    funevals = 0;
    return;
end
...
THat = zeros(MCreps,1,'like',outclass);
for rep = 1:MCreps
    ...
    % hypercube.
    w = abs(2*mod(qq + repmat(rand(size(q),'like',outclass),P,1), 1) - 1);
