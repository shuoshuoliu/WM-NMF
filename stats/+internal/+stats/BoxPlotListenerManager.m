classdef BoxPlotListenerManager < matlab.graphics.internal.SaveGraphicsVersion2Only 
    ...
    methods (Static)
        ...
        function repositionLabels(f,ax)
            ...
            for i = 1:length(visvalue)
                set(labelHandles(i),'Visible',visvalue{i});
            end
        ...
        function listeners = makeListeners(f, ax, labelAxis) 
            ...
            list6 = addlistener(ax, {'XTickLabel','XTick','YTickLabel','YTick'},'PostSet',@(src,evt) internal.stats.BoxPlotListenerManager.removeTickLabel(ax));

            listeners = {list1 list2 list3 list4 list5 list6};
        end
        ...
        function removeTickLabel(ax)
            boxparent = getappdata(ax,'boxplothandle');
            if isvalid(boxparent)
                labelAxis = getappdata(boxparent,'labelaxis');
                if (strcmpi(labelAxis,'x') && (isempty(ax.XTick)||isempty(ax.XTickLabel)))|| ...
                        (strcmpi(labelAxis,'y') && (isempty(ax.YTick)||isempty(ax.YTickLabel)))
                    labelPtsPos = getappdata(boxparent,'labelptspos');
                    labelHandles = getappdata(boxparent,'labelhandles');
                    vis = false(size(labelPtsPos));
                    visoptions = {'off';'on'};
                    visvalue = visoptions(vis+1);
                    for i = 1:length(visvalue)
                        set(labelHandles(i),'String','');
                    end                    
                end
            end
        end
