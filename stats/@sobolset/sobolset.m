...
%          Journal of Complexity, Vol. 14, pp. 527-556, 1998.
    ...
    methods
        function obj = reduceDimensions(obj, ndims)
        %REDUCEDIMENSIONS Create a new sobolset with fewer dimensions than the input
        %
        %  S2 = reduceDimensions(S, N) returns a sobolset object that is
        %  configured to only generate the first N dimensions from the
        %  input sobolset.  The point values returned from S2 will be equal
        %  to the first N elements of the point values from S:  S2(k,:) ==
        %  S(k,1:N).
        %
        %  N must be less than or equal to the number of dimensions in S.
        
        % Check that ndims is a finite positive integer and is <= the
        % number of dimensions in the input
        if ~isnumeric(ndims) || ~isscalar(ndims) || ~isreal(ndims) || ...
                ~isfinite(ndims) || ndims~=fix(ndims) || ...
                ndims<1 || ndims>obj.Dimensions
            error(message('stats:qrandset:InvalidNumberOfDimensions', obj.Dimensions))
        end
  
        obj.Dimensions = ndims;
        
        % Keep first ndims sets of direction numbers and scramble data
        obj.DirectionNumbers = obj.DirectionNumbers(1:ndims,:);
        if ~isempty(obj.DigitalShifts)
            obj.DigitalShifts = obj.DigitalShifts(1:ndims);
        end
    ...
    methods(Access = 'protected')
        ...
        X = internal.stats.sobolset.sobolPoint(obj.DirectionNumbers, indexSkipAndLeap(obj, Index) - 1, ...
            obj.GrayCodeSet);
        X = digitalShift(obj, X);
        X = internal.stats.sobolset.convertToDouble(X);
        end
        ...
        if obj.HasSkipLeap
            X = callSobolSequence(obj.DirectionNumbers, obj.Skip, obj.Leap, ...
                Count, obj.GrayCodeSet);
        else
            X = callSobolNet(obj.DirectionNumbers, Count, obj.GrayCodeSet);
        end
        X = digitalShift(obj, X);
        X = internal.stats.sobolset.convertToDouble(X);
        end
        ...
        X = internal.stats.sobolset.sobolIndexed(obj.DirectionNumbers, indexSkipAndLeap(obj, Idx)-1, ...
            obj.GrayCodeSet);
        X = digitalShift(obj, X);
        X = internal.stats.sobolset.convertToDouble(X);
        end
        ...
        % empty
        X = callSobolSequence(obj.DirectionNumbers, CorrectedIndex, L, ...
...
function X = callSobolSequence(dirnum,cidx,L,count,gcset,varargin)
if ~isreal(dirnum) || ~isreal(cidx) || ~isreal(L) || ~isreal(count) || ~isreal(gcset)
    error(message('stats:sobolset:NoComplex'));
end
X = internal.stats.sobolset.sobolSequence(dirnum,cidx,L,count,gcset,varargin{:});
end

function X = callSobolNet(dirnum,count,gcset)
if ~isreal(dirnum) || ~isreal(count) || ~isreal(gcset)
    error(message('stats:sobolset:NoComplex'));
end
X = internal.stats.sobolset.sobolNet(dirnum,count,gcset);
end