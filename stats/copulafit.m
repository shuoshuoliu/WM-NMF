...
case 'gaussian'
    ...
    z = norminv(u);
    RhoHat = corrcoef(z);
    RhoHat = (RhoHat+RhoHat')/2; %make the asymmetry of RhoHat (due only to
                                 %roundoff error for some situation) symmetric
    varargout{1} = RhoHat;
...
%
    function [nll,Rho,R] = profileNLL_t(nu)
        ...
        if isempty(Rsaved)
            Rho = corrcoef(t_);
            Rho = (Rho+Rho')/2; %make the asymmetry of Rho (due only to
                                %roundoff error) symmetric
            [R,p_] = chol(Rho);
    ...
    function [nll,Rho] = approxProfileNLL_t(nu)
        ...
        % Compute some log likelihood terms here instead of within the
        % function to improve performance. Not very useful for this call
        % but is useful later on.
        [n,dd] = size(t_);
        nllterms = - n*gammaln((nu+dd)/2) + n*dd*gammaln((nu+1)/2) - n*(dd-1)*gammaln(nu/2)...
            - ((nu+1)/2)*sum(log(1+t_.^2./nu),'all');

        % Compute negative log-likelihood at nu and conditional Rho estimate
        nll = negloglike_t(nu,chol(Rho),t_, nllterms);
...
function [obj,grad] = profileObjFun(params,nu,t)
    % Compute some log likelihood terms here instead of within the
    % function to improve performance. These terms are common to all calls
    % to negloglike_t
    [n,d] = size(t);
    nllterms = - n*gammaln((nu+d)/2) + n*d*gammaln((nu+1)/2) - n*(d-1)*gammaln(nu/2)...
        - ((nu+1)/2)*sum(log(1+t.^2./nu),'all');
    
    R = tomatrix(params);
    obj = negloglike_t(nu, R, t, nllterms);
    ...
    if nargout == 2
        ...
        for j = 1:length(params)
            ...
            grad(j) = negloglike_t(nu, Rplus, t, nllterms) - negloglike_t(nu, Rminus, t, nllterms);
            e(j) = 0;
        end
...
function nll = negloglike_t(nu, R, t, nllterms)
    ...
    tRinv = t / R;
    nll = nllterms + n*sum(log(abs(diag(R)))) ...
