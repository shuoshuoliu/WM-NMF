...
%   variable, vector, character array, a string array or cell array of
%   character vectors.  AOCTOOL displays three figures; one contains an
%   interactive graph of the data and prediction curves, another contains
%   an anova table, and a third contains parameter estimates.  The graph
%   figure can display predictions for all groups, or predictions and
%   confidence bounds for a single group.  The confidence level is
%   100(1-ALPHA) percent, with ALPHA=0.05 as the default.
%
...
if nargin > 6
    gname = convertStringsToChars(gname);
end
if nargin > 7
    displayopt = convertStringsToChars(displayopt);
end
if nargin > 8
