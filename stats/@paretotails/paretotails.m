...
%   Copyright 2006-2017 The MathWorks, Inc.
    ...
    methods(Hidden = true)
        % the following two things are defined as properties here and as
        % methods in separate files, and this needs to be maintained for
        % backward compatibility, so they cannot be hidden
        %         lowerparams(obj)
        %         upperparams(obj)
    end
    methods
        function out = get.NumSegments(this)
            out = nsegments(this);
        end
        function out = get.UpperParameters(this)
            out = this.upperparams;
        end
        function out = get.LowerParameters(this)
            out = this.lowerparams;
        end
    function b = paretotails(x,pLower,pUpper,cdfFunc)
        ...
        cdfFunc = convertStringsToChars(cdfFunc);
