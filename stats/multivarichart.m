...
%   of elements as Y.  The number of grouping variables must be 2, 3, or 4.
%
...
%
%   MULTIVARICHART(Y) displays the multivari chart for a matrix Y. The data
%   in different columns represent changes in one factor. The data in
...
%                    a string array or cell array of strings, one per
%                    grouping variable. Default names are 'X1', 'X2', ... .
%
%       'plotorder'  A string with the value 'sorted' or a vector
%                    containing a permutation of the integers from 1 to the
...
%           y = randn(100,1); % response group = [ceil(3*rand(100,1))
%           ceil(2*rand(100,1))]; multivarichart(y,group)
%
%      Display a multivari chart for data with four grouping variables.
%           y = randn(1000,1); % response group =
