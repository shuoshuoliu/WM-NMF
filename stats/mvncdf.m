...
elseif d <= 3
    ...
    if upperLimitOnly
        ...
        XL0(equalLims) = -Inf;
        XU0(equalLims) = -Inf;
        y = zeros(n,1,'like',internal.stats.dominantType(XL0,XU0,Rho));
        for i = 0:d
            k = nchoosek(1:d,i);
            for j = 1:size(k,1)
                X = XU0; X(:,k(j,:)) = XL0(:,k(j,:));
    ...
    if nargout > 1
        err = repmat(cast(tol,'like',y),size(y));
    end
