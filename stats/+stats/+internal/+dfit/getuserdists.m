function [news,newrows]=getuserdists(olds,userfun)
...
for j=1:numnewdists
   ...
   if newrow>numolddists
      oldnames = [oldnames {name}]; %#ok<AGROW>
      oldcodes = [oldcodes {code}]; %#ok<AGROW>
...
sj = stats.internal.dfit.probspecdefaults(sj);
