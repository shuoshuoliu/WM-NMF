...
switch linetype
 case 'data'
  ...
  for j=1:length(allmarkers)
     ...
     for k=1:size(allcolors,1)
        ...
        if isempty(a)
           for p=1:length(unplottedds)
              cml = get(unplottedds(p),'ColorMarkerLine');
 ...
 case 'fit'
  ...
  unplottedfit = [find(stats.internal.dfit.getfitdb,'plot',0); find(stats.internal.dfit.getfitdb,'plot',1)];
  ...
  for j=1:length(allstyles)
     ...
     for k=1:size(allcolors,1)
        ...
        a = findobj(h1,'flat','Color',c);
        if isempty(a)
           for p=1:length(unplottedfit) 
              cml = get(unplottedfit(p),'ColorMarkerLine');
              if iscell(cml) && ~isempty(cml) && ...
