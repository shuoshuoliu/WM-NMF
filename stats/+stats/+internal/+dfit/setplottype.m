function setplottype(dffig,ftype,dtype)
%SETPLOTTYPE Set plot type and distribution for distribution fitter
...
if isprobplot && (nargin<3 || isempty(dtype))
   ...
   if choice<=ntypes
      ...
      dtype = find(stats.internal.dfit.getfitdb, 'name', fname);
   end
elseif ~isprobplot
   dtype = [];
end
...
stats.internal.dfit.setfunction(dffig,ftype,dtype);
