function updatelegend(dffig,reset,leginfo)
%UPLDATELEGEND Update legend in dfittool window
...
for j=1:n
   ...
   if ds.showbounds
      ...
      if ~isempty(hbounds) && ishghandle(hbounds) ...
         ...
         textDataBounds{nextj,1} = [pre getString(message('stats:dfstrings:cellstr_ConfidenceBounds'))]; %#ok<AGROW>
         hDataBounds = [hDataBounds; hbounds]; %#ok<AGROW>
         sortDataBounds(nextj,1) = sortData(j) + .5; %#ok<AGROW>
         nextj = nextj + 1;
      end
   end
end
...
set( h(tf), 'Callback', @(s, e) stats.internal.dfit.togglelegend(gcbf, 'off'));
...
set( h(tf), 'Callback', @(s, e) stats.internal.dfit.updatelegend(gcbf, true ) );
