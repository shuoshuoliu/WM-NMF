function tips(~,eventData)
%TIPS Display data and fit tips for Distribution Fitter figure
...
% Otherwise we need to create the proper label
   ...
   if isempty(dftip) || ~ishghandle(dftip)
      ...
      dftip = text(x,y,'','Color','k','VerticalAlignment','bottom',...
                          ...
                          'HitTest','off','PickableParts','none','FontWeight','bold',...
                          'Backgroundcolor',yellow,'Margin',3,...
                          'Edgecolor','k');
   end
   if isempty(dfdot) || ~ishghandle(dfdot)
      dfdot = line(x,y,'Marker','o','LineStyle','none','Color','k',...
                       'Tag','dfdot','Parent',ax,'HitTest','off','PickableParts','none');
   end
   ...
   set(dffig, 'WindowButtonMotionFcn',@stats.internal.dfit.tips);
