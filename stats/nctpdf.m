...
nans = ~((nu > 0) & isfinite(delta)) | isnan(x);
y(nans) = NaN;

% Use normal approximation when nu>1e6
bignu = nu>1e11 & ~nans;
if any(bignu(:))
    s = 1 - 1./(4*nu);
    d = sqrt(1+x.^2./(2*nu));
    y(bignu) = normpdf(x(bignu).*s(bignu),delta(bignu),d(bignu));
end

% Negative data -- use left tail of CDF to get PDF.  -Infs return zero.
k = find((x < 0) & isfinite(x) & ~bignu & ~nans);
if any(k)
    y(k) = (nu(k)./x(k)) .* ...
